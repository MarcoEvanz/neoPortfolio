...
export default function SNESEmulatorApp() {
  ...
  const handleUrlLoad = useCallback(() => {
    ...
    if (!url) return;
    const name = url.split("/").pop()?.replace(/\.[^.]+$/, "") || "SNES Game";
    loadRom(url, name);
  }, [urlInput, loadRom]);
      ...
      <div className="flex flex-col h-full bg-[#0a0f1a]">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-6 px-6">
          <div className="w-full max-w-md space-y-2">
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="https://example.com/game.sfc"
                className="flex-1 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm placeholder:text-white/20 outline-none focus:border-purple-500/50 transition-colors"
              />
              <button
                onClick={handleUrlLoad}
                disabled={!urlInput.trim()}
                className="px-4 py-2 rounded-lg bg-purple-500/20 border border-purple-500/30 text-purple-400 text-sm font-medium hover:bg-purple-500/30 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
              >
                Load
              </button>
            </div>
          ...
          <div className="w-full max-w-md">
            ...
            <label className="flex flex-col items-center justify-center gap-2 py-8 rounded-xl border-2 border-dashed border-white/10 hover:border-purple-500/30 cursor-pointer transition-colors">
              <HiUpload className="text-2xl text-white/20" />
              <span className="text-xs text-white/30">Click to select a .sfc, .smc, or .zip file</span>
              <input
                type="file"
                accept=".sfc,.smc,.zip,.7z"
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0f1a]">
      <div className="shrink-0 px-4 pt-4 pb-3 border-b border-white/5">
        <h1 className="text-lg font-bold text-white mb-1">SNES Emulator</h1>
        <p className="text-[11px] text-white/30">Play Super Nintendo games with EmulatorJS</p>
      </div>
      <div className={`flex-1 min-h-0 overflow-y-auto px-4 py-3 ${isMobile ? "pb-6" : ""}`}>
        <button
          onClick={() => setView("loader")}
          className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-gradient-to-r from-purple-500/10 to-violet-500/10 border border-purple-500/20 hover:border-purple-500/40 transition-colors mb-4"
        >
        ...
        {bundledRoms.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 gap-3">
            ...
            <p className="text-[11px] text-white/15 text-center max-w-[200px]">
              Add .sfc or .smc files to /public/snes/ and restart, or use the custom loader above
            </p>
          </div>
        )}
      </div>
